...
class Digit:
	...
	def process_webcam(self):
		...
		while image[0][0][0] == -1 and trying_to_find_index_card:
			...
			sleep(2)
			_, image = webcam.read()
			print("Image Taken... autocropping now")
		...
		if trying_to_find_index_card: 
			...
			self.first = out2
			self.first_percentage = prediction_weights2[0][out2 - 1] * 100
			self.second = next
...
while True:
    ...
    if key == 99:
    	digit = Digit()
    	if (digit.process_webcam()):
    		most_likely = digit.first
    		percent_likely = digit.fist_percent
    		print("Success... Guessed",most_likely,"with",percent_likely,"percent confidence")
    	else: print("Failure...")
